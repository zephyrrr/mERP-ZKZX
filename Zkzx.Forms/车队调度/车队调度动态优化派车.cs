...
{
    public partial class 车队调度动态优化派车 : Feng.Windows.Forms.MyTemplateForm, IRefreshDataForm
    {
        private DataUnboundGrid m_待命车辆_单车单任务Grid;
        ...
        private void btn计算机辅助优化_Click(object sender, EventArgs e)
        {
            ...
            {
                选择组合(m_待排任务Grid, m_待命车辆_单车单任务Grid);
            }
            else
            {
                选择组合(m_待命车辆_单车单任务Grid, m_待排任务Grid);
            }
        }

        private void Clear组合()
        {
            专家调度一级静态优化.Clear组合(m_待命车辆_单车单任务Grid, m_待排任务Grid);
        }
        ...
        {
            m_待命车辆_单车单任务Grid.DisplayManager.SearchManager.LoadData();
            m_待排任务Grid.DisplayManager.SearchManager.LoadData();
        }

        private void btn批量确认_Click(object sender, EventArgs e)
        {
            Helper.btn批量确认_Click(m_待命车辆_单车单任务Grid, (sender1, e1) =>
                {
